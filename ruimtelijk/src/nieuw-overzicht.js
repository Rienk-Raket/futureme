...
function nwoThemas() {
  ...
  const pogingen = S.gebeurtenissen.filter(g => /^Gestopt met roken sinds/.test(g.tekst || "")).length || (rk ? 1 : 0);
  ...
  const gez = [];
  if (rk) gez.push(r("Dagen gestopt met roken", n(rk.heleDagen), `${eur(rk.bespaard)} bespaard · ${n(rk.sigaretten)} sigaretten niet`));
  if (pogingen) gez.push(r("Stoppogingen", n(pogingen)));
