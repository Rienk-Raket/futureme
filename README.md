...
| `ruimtelijk/src/` | De toegevoegde lagen als losse bronbestanden: `ruimte.css/js` (glas, diepte, overgangen), `hobbyskills.css/js` (HobbySkills), `hs-sjablonen.js` (checklist-sjablonen per soort hobby of skill), `mm-bron.js` (hobby's, skills en side hustles als mindmap, met synchroon afvinken), `koppelingen.js` (Verder naar, gegroepeerd Meer-scherm, widgets, scrollpositie van filterbalken) |
