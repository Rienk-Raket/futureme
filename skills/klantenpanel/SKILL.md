...
1. **Materiaal en opzet.** Zet elke versie als eigen bestand in `$RUN/ronde-N/materiaal/` en schrijf `$RUN/ronde-N/opzet.json`:
   ...
   {"objecten": [{"id": "origineel", "titel": "Landingspagina", "bestand": "ronde-1/materiaal/origineel.md"}],
    ...
    "vragen_extra": []}
   ```
   Object-ids kort en zonder spaties (`origineel`, `v1`, `A`, `concept-1`). Zet in `context` de prijseenheid voor ronde 2, anders zijn de Van Westendorp-antwoorden onvergelijkbaar. Houd de context tussen de rondes gelijk (zelfde kanaal, zelfde eenheid, zelfde aannames); wat je in ronde 1 als vanzelfsprekend meegaf, moet in ronde 4 nog steeds gelden. Het sjabloon zegt de panelleden al dat het aanbod in hun eigen woonplaats beschikbaar is; noem een plaats in `context` dus alleen als de plaats zelf getest moet worden. Bevat een verbeterde versie die jij schrijft invulgegevens (naam, KvK-nummer, telefoonnummer, klantcitaat), zeg dan in `context` dat die als echt en kloppend gelden; anders beoordelen panelleden de invulwaarden in plaats van het aanbod.
2. **Prompts bouwen.** `python3 <skill>/scripts/build_prompt.py --run $RUN --ronde N --alle` schrijft per persona `ronde-N/prompts/<id>.md`.
3. **Subagents starten, allemaal in dezelfde beurt.** Eén subagent per persona (`general-purpose`, op de achtergrond), met als volledige opdracht: "Lees `<absoluut pad>/ronde-N/prompts/<id>.md` en voer de opdracht daarin uit. Schrijf alleen het gevraagde JSONL-bestand." Wacht op de meldingen; start de subagents niet één voor één en vat de prompt niet samen in de opdracht. Verwijder vóór een herhaling het oude `ronde-N/reacties/<id>.jsonl` van die persona. Blijkt een hele ronde mislukt door een fout in jouw opzet (bijvoorbeeld een context die iedereen buiten spel zet), verplaats dan `reacties/` naar `reacties-poging-1/` en `opzet.json` naar `opzet-poging-1.json`, herstel de opzet, draai de ronde opnieuw en meld dat in de samenvatting. Zo blijft de mislukte poging herleidbaar.
